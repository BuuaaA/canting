...
jobs={
 ...
 'widget':flutter+['test','--no-pub','--concurrency=1','test/state/p6a_widget_date_test.dart','test/state/p5_widget_sync_test.dart'],
 'format':['C:/flutter/bin/cache/dart-sdk/bin/dart.exe','format','lib/state/app_state.dart','test/state/p6a_widget_date_test.dart','lib/data/food_database.dart','lib/data/database_helper.dart','lib/data/custom_dish_repository.dart','lib/core/models/portions.dart','lib/ui/about/about_page.dart','test/data/ponytail_regression_test.dart'],
 'full':flutter+['test','--no-pub','--concurrency=1','--reporter','expanded'],
 'analyze':flutter+['analyze','--no-pub'],
 'kotlin':[str(r/'android/gradlew.bat'),':app:cleanTestDebugUnitTest',':app:testDebugUnitTest','--offline','--no-daemon','--max-workers=1','-Dorg.gradle.jvmargs=-Xmx2G -XX:MaxMetaspaceSize=1G'],
}
name=sys.argv[1]
build_names=['baseline-offline','universal','split','bundle']
if name in build_names:
 jobs[name]=[str(r/'android/gradlew.bat'), ':app:bundleRelease' if name=='bundle' else ':app:assembleRelease', '--offline','--no-daemon','--max-workers=1','-Dorg.gradle.jvmargs=-Xmx2G -XX:MaxMetaspaceSize=1G','-Ptarget=lib/main.dart','-Ptarget-platform=android-arm,android-arm64,android-x64','-Ptree-shake-icons=true','-Pforce-version-code-ignoring-abi=true']+(['-Psplit-per-abi=true'] if name=='split' else [])
...
if p.returncode==0 and name in ['baseline',*build_names]:
 dest=r/'build/p6a-artifacts'/run_id/name;dest.mkdir(parents=True,exist_ok=True)
