  ...
  Widget build(BuildContext context) => FutureBuilder<TodayIntakeStats>(
    ...
    builder: (context, snapshot) {
      ...
      if (snapshot.hasError) {
        return PixelPanel(
          child: _MessageState(message: '今日餐盘加载失败', action: _retry),
        );
      }
      ...
      return Column(
        children: [
          PixelPanel(
            ...
            child: Column(
              children: [
                for (final stat in stats.categories.values)
...
class _CategoryRow extends StatelessWidget {
  ...
  String _amount() => stat.amount == null
      ? (stat.completeness == 'missing' ? '未记录' : '未知')
      : '${_n(stat.amount!)}${stat.unit}';
  String _actual() {
    if (stat.actualKnownByUnit.isEmpty) return '';
    final entries = stat.actualKnownByUnit.entries
        .map((entry) => '${_n(entry.value)}${entry.key}')
        .join('、');
    return '已记录$entries；';
  }

  String _target() {
    final min = stat.target.min, max = stat.target.max;
    if (min != null && max != null) return '${_n(min)}～${_n(max)}${stat.unit}';
    if (min != null) return '至少${_n(min)}${stat.unit}';
    if (max != null) return '不超过${_n(max)}${stat.unit}';
    return '暂无参考';
  }

  String _status() => switch (stat.status) {
    'below' => '需补充',
    'near' => '接近下限',
    'met' => '已达标',
    'high' => '偏多',
    _ => stat.completeness == 'missing' ? '未记录' : '暂不能判断',
  };
  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final color = stat.status == 'met'
        ? theme.colorScheme.primary
        : theme.colorScheme.onSurfaceVariant;
    final actualText = stat.amount == null
        ? '${_actual()}暂不能比较'
        : '${_actual()}${_amount()}';
    return Padding(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(actualText),
                Text('参考 ${_target()}', style: theme.textTheme.labelSmall),
